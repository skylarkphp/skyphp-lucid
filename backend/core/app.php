  ...
  if($_GET['section'] == "fetch")
	{
		if($_GET['action'] == "full")
		{
			if(isset($_POST['filename'])) {
				...
				$file = $GLOBALS['path']."/../desktop/openstar/apps/".$_POST['filename'];
				...
				$files=array();
				$files[$_POST['sysname']] = jsSearch($GLOBALS['path']."/../desktop/openstar/apps/".$_POST['sysname'],
									$GLOBALS['path']."/../desktop/openstar/apps/");
		...
		if($_GET['action'] == "listAll")
		{
			...
			{
				$item = array();
				foreach(array("sysname", "name", "author", "email", "maturity", "category", "version", "filetypes", "compatible",
					"icon","download_count","view_count","description","fav_count","created_at","updated_at") as $key) {
					$item[$key] = $v->$key;
				}
				$item["files"] = array();
				if(is_dir($GLOBALS['path']."/../desktop/openstar/apps/".$item['sysname']))
					$item["files"][$v->sysname] = jsSearch($GLOBALS['path']."/../desktop/openstar/apps/".$v->sysname,
												$GLOBALS['path']."/../desktop/openstar/apps/");
			...
			$cur = $User->get_current();
			$app_users = $App_User->filter("user_id",$cur->id);
			...
			foreach($p as $d => $v)
			{
				...
				foreach($app_users as $app_user){
					if($item["app_id"] == $app_user->app_id){
						$item["current_user_app"] = true;
					}
				}
	...
	if($_GET['section'] == "write")
	{
        ...
        if(!array_key_exists("sysname", $_POST)){
			...
			if(isset($_POST['filename'])) {
				$_POST['filename'] = str_replace("..", "", $_POST['filename']);
				file_put_contents($GLOBALS['path']."/../desktop/openstar/apps/".$_POST['filename'], $_POST['content']);
			}
			$out = new jsonOutput(array("status" => "ok","id" => $app->id, "download_count" =>$app->download_count));
		...
		if($_GET['action'] == "createFolder") {
			...
			mkdir($GLOBALS['path']."/../desktop/openstar/apps/".$_POST['dirname'], 0777);
		...
		if($_GET['action'] == "remove") {
			...
			if($_POST['sysname']) {
				...
				foreach(array(
					...
					$GLOBALS['path']."/../desktop/openstar/apps/".$_POST['sysname'],
					$GLOBALS['path']."/../desktop/openstar/apps/".$_POST['sysname'].".js"
				) as $dir) {
				...
				$dir = $GLOBALS['path']."/../desktop/openstar/apps/".$_POST['filePath'];
				if(is_dir($dir)) rmdir_recurse($dir);
				else if(is_file($dir)) unlink($dir);
			}
			$out = new intOutput("ok");
		...
		if($_GET['action'] == "rename") {
			...
			$_POST['origName'] = $GLOBALS['path']."/../desktop/openstar/apps/"
				. str_replace("..", "", $_POST['origName']);
			$_POST['newName'] = $GLOBALS['path']."/../desktop/openstar/apps/" 
