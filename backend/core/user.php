...
*/
  ...
  // echo 111;
	...
	// 	{
	// 		$user = $User->get_current();
	// 		$app_users = $App_User->filter("user_id",$user->id);
	// 		var $app_ids = array();
	// 	}
	// }
	...
	if($_GET['section'] == "apps")
	{
		...
		{
			$user = $User->get_current();
			$app_user = new $App_User(array("user_id" => 8, "app_id" => 4));
			$app_user->save();
			$app_users = $App_User->filter("user_id",$user->id);
			$app_ids = array();
			$out = new jsonOutput();
		...
		{
			$user = $User->get_current();
			foreach($_POST["app_ids"] as $app_id){
				$app_user = new App_User(array("user_id" => $user->id,"app_id" => $app_id));
				$app_user->save();
			}
		}
	}
