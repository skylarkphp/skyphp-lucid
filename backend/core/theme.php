...
if($_GET['section'] == "get")
{
	if($_GET['action'] == "list")
	{
	  $dir = opendir($GLOBALS['path']."/../desktop/qface/resources/themes/");
		$p = array();
		while(($file = readdir($dir)) !== false) {
			if($file == '..' || $file == '.' || $file{0} == '.'){
				...
				$t = strtolower($file);
				$theme = $GLOBALS['path']."/../desktop/qface/resources/themes/" . $file;
				if(is_dir($theme)){
					$m = $theme.'/meta.json';
					if (file_exists($m)) {
						$json = file_get_contents($m);
...
if($_GET['section'] == "package" && $cur->has_permission("core.administration")) {
	...
	if($_GET['action'] == "remove") {
		...
		rmdir_recurse($GLOBALS['path']."/../desktop/qface/resources/themes/".$name);
