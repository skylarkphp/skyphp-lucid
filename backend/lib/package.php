...
class package {
    function compatible($versions) {
        $data = file_get_contents($GLOBALS['path']."../desktop/openstar/resources/version.json");
	...
	function _install_application($info, $path) {
		...
		$sysDir = $GLOBALS['path']."../desktop/openstar/apps/".$app->sysname;
		if(is_dir($path."/".$app->sysname)) package::_recursive_copy($path."/".$app->sysname, $sysDir);
		$appFile = $GLOBALS['path']."../desktop/openstar/apps/".$app->sysname.".js";
		copy($path."/".$app->sysname.".js", $appFile);
		return array(
			"/apps/".$app->sysname,
			"/desktop/openstar/apps/".$app->sysname,
			"/desktop/openstar/apps/".$app->sysname.".js"
		);
	}
