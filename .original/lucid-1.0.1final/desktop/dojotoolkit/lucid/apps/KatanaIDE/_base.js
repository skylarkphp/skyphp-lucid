dojo.provide("lucid.apps.KatanaIDE._base");

dojo.declare("lucid.apps.KatanaIDE", lucid.apps._App, {
	...
	init: function(args){
		...
		lucid.app.list(dojo.hitch(this, function(apps){
			for(var i in apps){
...
//			this.toolbar.addChild(new dijit.form.Button({label: sys.kill, iconClass: "icon-16-actions-media-playback-stop", onClick: dojo.hitch(this, "killExec")}));
					...
					onComplete: dojo.hitch(this, function(dirname){
						...
						this.appStore.fetch({
							...
							onComplete: dojo.hitch(this, function(items){
								if(items.length != 0) return lucid.dialog.notify({
									type: "warning",
									message: nf.alreadyExists
								})
								lucid.app.createFolder(path+"/"+dirname);
					...
					onComplete: dojo.hitch(this, function(filename){
						...
						this.appStore.fetch({
							...
							onComplete: dojo.hitch(this, function(items){
								...
								var defaultContent = "dojo"+".provide(\"lucid.apps."
		...
		menu.addChild(new dijit.MenuItem({
			...
			onClick: dojo.hitch(this, function(){
				...
				var doDelete = dojo.hitch(this, function(){
					lucid.app.remove(fname ? null : sname, fname || null, dojo.hitch(this, function(result){
						if(!result) return;
	...
	makeTab: function(appname, filename, content){
		...
		this.store.fetch({
			...
			onComplete: dojo.hitch(this, function(items){
				...
				var editor = new lucid.apps.KatanaIDE[this.store.getValue(item, "value") ? "EditorLite" : "CodeTextArea"]({
				    plugins: "BlinkingCaret GoToLineDialog Bookmarks MatchingBrackets",
				    resizeTo: pane.id,
				    colorsUrl: dojo.moduleUrl("lucid.apps.KatanaIDE.data", "javascript_dojo_color.json"),
				    autocompleteUrl: dojo.moduleUrl("lucid.apps.KatanaIDE.data", "javascript_dojo_ac.json")
			    }, div);
			    pane.ide_info = {
				    fileName: filename,
				    appName: appname,
	...
	_makeMetaDialog: function(){
		var mnu = dojo.i18n.getLocalization("lucid.ui", "menus");
		var sys = dojo.i18n.getLocalization("lucid", "system");
		var cmn = dojo.i18n.getLocalization("lucid", "common");
	...
	_newApp: function(info){
		...
		var defaultContent = "dojo"+".provide(\"lucid.apps."+info.sysname+"\");\r\n\r\n"
								+"dojo.declare(\"lucid.apps."+info.sysname+"\", lucid.apps._App, {\r\n"
								+"	init: function(args){\r\n"
		...
		lucid.app.save(dojo.mixin(dojo.clone(this.appInfo[info.sysname]), {
	...
	onItem: function(item){
		...
		lucid.app.get(app, filename, dojo.hitch(this, function(content){
