dojo.provide("lucid.apps.AdminPanel.themes");
...
dojo.extend(lucid.apps.AdminPanel, {
	themes: function(){
		...
		lucid.theme.list(dojo.hitch(this, function(data){
			for(var i=0;i<data.length;i++){
				var src = dojo.moduleUrl("lucid.resources.themes."+data[i].sysname, data[i].preview);
			...
			dojo.connect(this._themeStore, "onDelete", this, function(a){
				lucid.theme.remove(a.sysname[0]); //that feels really hackish
			})
	...
	installThemePackage: function(){
		...
		var uploader = new dojox.widget.FileInputAuto({
			name: "uploadedfile",
			url: lucid.xhr("core.theme.package.install"),
