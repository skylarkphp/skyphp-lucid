...
var kDefaultURL = 'openstar/apps/PDFViewer/test_file/f1040.pdf';
...
var kMinScale = 0.25;
var kMaxScale = 4.0;
var kImageDirectory = 'openstar/apps/PDFViewer/templates/images/';
...
PDFJS.workerSrc = 'openstar/apps/PDFViewer/templates/javascripts/build/pdf.js';
...
var PDFView = {
  ...
  setTitleUsingUrl: function pdfViewSetTitleUsingUrl(url) {
    this.url = url;
    try {
      // document.getElementById('pdfViewer').title = decodeURIComponent(getFileName(url)) || url;
    } catch (e) {
      // decodeURIComponent may throw URIError,
      // fall back to using the unprocessed url in that case
      // document.getElementById('pdfViewer').title = url;
    }
  },
  ...
  load: function pdfViewLoad(pdfDocument, scale) {
    ...
    pdfDocument.getMetadata().then(function(data,url) {
      ...
      if (!pdfTitle && info && info['Title'])
        pdfTitle = info['Title'];
      //change PDFViewer's title
      // var pdf_viewer = dojo.query('#pdfViewer_title')[0];
      // if (pdfTitle)
      //   //document.title = pdfTitle + ' - ' + document.title;
      //   pdf_viewer.innerHTML = pdf_viewer.innerHTML + ' ' + pdfTitle;
      // else
      //   pdf_viewer.innerHTML = pdf_viewer.innerHTML ;
    });
  },
...
// document.addEventListener('DOMContentLoaded', function webViewerLoad(evt) {
  ...
  require(["dojo/on"], function(on){
    on(dojo.byId('openFile'),'click',function(){
      openstar.ui.dialog.file({
        ...
        onComplete: function (file){
          ...
          srvFilesystem.readFileContents(file, function(content){
          });
        }
      });
      // on(dojo.byId('fileInput'),'click',function(){});
    });

    on(dojo.byId('fullscreen'),'click',function(){
      PDFView.fullscreen();
    });
  });
