define("openstar/services/crosstalk",[
   	 "dojo", // dojo
   	 "require",
   	 "dojo/topic",
 	...
 	"./user",
	"./config"
],function(dojo,require,topic,Runtime,srvGeneral,srvUser,srvConfig) {

	var crosstalk = dojo.getObject("openstar.services.crosstalk",true);

	dojo.mixin(crosstalk, {
			//	summary:
			...
			subscribe: function(/*String*/topic, /*Function*/handler, /*Int?*/instance)	{
				...
				var session = crosstalk.session;
				session[session.length] = {
					appsysname: (instance ? openstar.instances[instance].sysname : -1), //TODO will be modified
					callback: handler,
					instance: instance || -1,
					topic: topic
				};
			...
			unsubscribe: function(/*Handle*/handle)	{
				...
				delete crosstalk.session[handle];
			},
			_internalCheck: function(){
				...
				//	//Runtime.log("Crosstalk API: No events to process...");
				//	this.setup_timer();
				//}
				//else { // handlers found. ask to obtain any events.
					//Runtime.log("Crosstalk API: Checking for events...");
					srvGeneral.xhr({
			        	...
			        	error: function(type, error){ Runtime.log("Error in Crosstalk call: "+error.message); }
		        	});
				//}
			},
			exists: function(/*Int?*/id, /*Function*/onComplete, /*Function?*/onError){
		        ...
		        srvGeneral.xhr({
			    	...
			    	error: function(type, error){
						Runtime.log("Error in Crosstalk call: "+error.message);
		                d.errback(error);
						this.setup_timer();
				    }
			    });
				...
				try {
		            ...
		            dojo.forEach(data, function(event, key){
						...
						if(handled == false && (event.instance == -1 || event.instance == "null")){
							...
							topic.publish("openstar/services/crosstakl/launchApp",event.appsysname, {crosstalk: true}, dojo.hitch(this, function(app){
			...
			setup_timer: function()	{
		        ...
		        var time = srvConfig.crosstalkPing || 100000;
