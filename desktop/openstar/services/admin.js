define("openstar/services/admin",[
   	 "dojo", //dojo
   	 "./general"
],function(dojo,srvGeneral) {
	var admin = dojo.getObject("openstar.services.admin",true);
	dojo.mixin(admin, {
	    init: function(callback){
	    	srvGeneral.xhr({
	            backend: "core.user.info.isAdmin",
	            load: function(data){
	                openstar.services.admin.isAdmin = data.isAdmin;
	                if (callback) {
	                	callback();
	                }
	            },
