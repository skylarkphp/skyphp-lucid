define([
...
],function(dojo,require,ItemFileReadStore,topic,_Widget,_Templated,_Container,_Contained,TextBox,Button,Menu,MenuItem,MenuSeparator,PopupMenuItem,Tree,_LayoutWidget,ContentPane,FilteringSelect,Runtime,Window,dialog,utilHtml,srvFileSystem,srvConfig,srvApp,nlsCommon,nlsApps,nlsMessages,nlsFileArea) {
	var _fileareaClipboard = {
	...
	var FileArea = dojo.declare([_LayoutWidget], {
		//	path: String
		...
		postCreate: function(){
			...
			var nf = nlsFileArea;//i18n.getLocalization("scene.widget", "filearea",this.lang);
			...
			menu.addChild(new MenuItem({label: nf.createFolder, iconClass: "icon-16-actions-folder-new", onClick: dojo.hitch(this, this._makeFolder)}));
			menu.addChild(new MenuItem({label: nf.createLauncher, onClick: dojo.hitch(this, this._makeLauncher)}));
			menu.addChild(new MenuItem({label: nf.createFile, iconClass: "icon-16-actions-document-new", onClick: dojo.hitch(this, this._makeFile)}));
			menu.addChild(new MenuSeparator({}));
			menu.addChild(new MenuItem({label: cm.refresh, iconClass: "icon-16-actions-view-refresh", onClick: dojo.hitch(this, this.refresh)}));
			menu.addChild(new MenuSeparator({}));
			menu.addChild(new MenuItem({
				label: nf.paste,
				iconClass: "icon-16-actions-edit-paste",
				onClick: dojo.hitch(this, function(){
					var clip = _fileareaClipboard;
					if(!clip) return;
					if(clip.type == "") return;
					...
					if(clip.type == "cut"){
						...
						if(isParent(this.path+name, clip)) return dialog.notify({message: nf.parentErr, type: "warning", duration: 5000,scene:this.scene});
						this._loadStart();
						if(clip.widgetRef && clip.widgetRef.getParent().path == this.path) return;
						srvFileSystem.move(clip.path+clip.name, this.path+name, dojo.hitch(this, function(){
							var parentID;
							if(clip.widgetRef){
					...
					if(clip.type == "copy"){
						...
						if(isParent(this.path+name, clip)) return dialog.notify({message: nf.parentErr, type: "warning", duration: 5000,scene:this.scene});
						this._loadStart();
						srvFileSystem.copy(clip.path+clip.name, this.path+name, dojo.hitch(this, function(){
							this._loadEnd();
							this.refresh();
		...
		onItem: function(/*String*/path)
		{
			...
			this.launchHandler(path);
		},
		
		launchHandler : function(path){
			topic.publish("openstar/system2/Runtime/launchHandler",path);
		},
		...
		_makeFolder: function(){
			...
			dialog.input({
				...
				onComplete: dojo.hitch(this, function(dirname){
					...
					srvFileSystem.createDirectory(this.path+dirname, dojo.hitch(this, function(){
						dojo.publish("filearea:"+this.path, [this.id]);
						this.refresh();
					}));
				})
		...
		_makeLauncher: function(){
			var nf = nlsFileArea;//i18n.getLocalization("scene.widget", "filearea",this.lang);
			var cm = nlsCommon;//i18n.getLocalization("scene", "common",this.lang);
			var appNls = nlsApps;//i18n.getLocalization("scene", "apps",this.lang);
			var win = new Window({
			...
			utilHtml.textContent(row0, nf.name+": ");
			var nameBox = new TextBox({});
			row0.appendChild(nameBox.domNode);
			div.appendChild(row0);
			
			var row1 = document.createElement("div");
			utilHtml.textContent(row1, nf.application+": ");
			var appItems = [];
			dojo.forEach(srvApp.appList, function(item){
				appItems.push({
					label: appNls[item.name] || item.name,
					value: item.sysname
				});
			})
			var appBox = new FilteringSelect({
				autoComplete: true,
				searchAttr: "label",
			...
			utilHtml.textContent(row2, nf.arguments+": ");
			...
			var okButton = new Button({
				...
				onClick: dojo.hitch(this, function(){
					...
					srvFileSystem.writeFileContents(
						this.path+this._fixDuplicateFilename(name+".scene", "text/plain"),
						app+"\n"+dojo.toJson(args),
		...
		_makeFile: function(){
			...
			var nf = nlsFileArea;//dojo.i18n.getLocalization("scene.widget", "filearea");
			dialog.input({
				title: nf.createFile,
				...
				onComplete: dojo.hitch(this, function(filename){
					...
					srvFileSystem.writeFileContents(this.path+filename, "", dojo.hitch(this, function(){
						dojo.publish("filearea:"+this.path, [this.id]);
						this.refresh();
					}));
				})
...
//				w.menu._contextMouse();
			w.menu._openMyself({"target":e.target, "coords":{x: e.pageX, y: e.pageY}});
			}
			else
			{
//				this.menu._contextMouse();
		...
		refresh: function(/*String?*/id, /*String?*/targetid){
			...
			//list the path
			this._loadStart();
			this._lsHandle = srvFileSystem.listDirectory(this.path, dojo.hitch(this, function(array){
				this._lsHandle = null;
	...
	var _Icon = dojo.declare([_Widget, _Templated, _Contained], {
		templateString:"<div class='fileItem' style='width: 80px; height: 50px; padding: 10px;' dojoAttachPoint='focusNode'>\n\t<div class='fileIcon ${iconClass}' dojoAttachPoint='iconNode'></div>\n\t<div class='iconLabel' dojoAttachPoint='labelNode'>\n\t\t<div dojoAttachPoint='textFront' class='shadowFront'>${label}</div>\n\t\t<div dojoAttachPoint='textBack' class='shadowBack'>${label}</div>\n\t\t<div dojoAttachPoint='textHidden' class='shadowHidden'>${label}</div>\n\t</div>\n</div>\n",
		//	label: string
		...
		postCreate: function(){
			var nc = nlsCommon;//dojo.i18n.getLocalization("scene", "common");
			var nf = nlsFileArea;//dojo.i18n.getLocalization("scene.widget", "filearea");
			...
			menu.addChild(new MenuItem({label: nc.open, iconClass: "icon-16-actions-document-open", onClick: dojo.hitch(this, "_onDblClick")}));
	        menu.addChild(new MenuItem({label: nf.openWith, onClick: dojo.hitch(this, "_openWith")}));
				var menuDl = new PopupMenuItem({iconClass: "icon-16-actions-document-open", label: nc.download});
				var menu2 = new Menu({parentMenu: menuDl});
				menu2.addChild(new MenuItem({label: nf.asFile, onClick: dojo.hitch(this, function(e){
					srvFileSystem.download(this.getParent().path+this.name);
				})}));
				menu2.addChild(new MenuItem({label: nf.asZip ,onClick: dojo.hitch(this, function(e){
					srvFileSystem.download(this.getParent().path+this.name, "zip");
				})}));
				menu2.addChild(new MenuItem({label: nf.asTgz, onClick: dojo.hitch(this, function(e){
					srvFileSystem.download(this.getParent().path+this.name, "gzip");
				})}));
				menu2.addChild(new MenuItem({label: nf.asTbz2, onClick: dojo.hitch(this, function(e){
					srvFileSystem.download(this.getParent().path+this.name, "bzip");
				})}));
				menu2.startup();
				menuDl.popup = menu2;
			...
			menu.addChild(new MenuSeparator({}));
			menu.addChild(new MenuItem({label: nc.rename, iconClass: "icon-16-apps-preferences-scene-font", onClick: dojo.hitch(this, "rename")}));
			menu.addChild(new MenuItem({label: nc["delete"], iconClass: "icon-16-actions-edit-delete", onClick: dojo.hitch(this, "deleteFile")}));
			menu.startup();
		},
		...
		_onRelease: function(e){
			...
			if(newTarget.isInstanceOf && (newTarget.isInstanceOf(FileArea) || newTarget.isInstanceOf(_Icon))) 
				var targetPath = (newTarget.isInstanceOf(_Icon)
			...
			if (this._docNode){
				...
				if (srvConfig.fx > 0 &&
				(!(newTarget.isInstanceOf(FileArea) || newTarget.isInstanceOf(_Icon)) || isParent(targetPath))){
				...
				var nf = nlsFileArea;//i18n.getLocalization("scene.widget", "filearea");
				if(isParent(targetPath)) dialog.notify({message: nf.parentErr, type: "warning", duration: 5000,scene:this.scene});
				if(((e.shiftKey && newTarget.type != "text/directory") || newTarget.id != this.getParent().id)
				&& newTarget.id != this.id
				&& !isParent(targetPath)
				&& (newTarget.isInstanceOf(FileArea) || newTarget.isInstanceOf(_Icon))){
					var _loadParent = this.getParent();
					_loadParent._loadStart();
					if (e.shiftKey){
						//copy the file
						if(newTarget.isInstanceOf(FileArea)) 
						...
						srvFileSystem.copy(this.getParent().path + "/" + this.name, targetPath + name, function(){
							_loadParent._loadEnd();
							if(newTarget.isInstanceOf(FileArea)) newTarget.refresh();
						...
						if(newTarget.isInstanceOf(FileArea)) 
							...
							var name = (newTarget.type == "text/directory" ? 
										this.name : //TODO: fix the name server side? or at least show an error message...
										newTarget.getParent()._fixDuplicateFilename(this.name, this.type));
						srvFileSystem.move(this.getParent().path + "/" + this.name, targetPath + name, dojo.hitch(this, function(){
							_loadParent._loadEnd();
		...
		_formatLabel: function(name){
			var p = name.lastIndexOf(".");
			var ext = name.substring(p+1, name.length);
			if((srvConfig.filesystem.hideExt && this.type != "text/directory" && p != -1)
			|| ext == "scene"){
	    ...
	    _openWith: function(){
	        ...
	        var win = this._win = new Window({
	            title: nf.openWith,
	            height: "350px",
	            width: "300px"
	        });
	        var p = new ContentPane({region: "top"});
	    ...
	    _makeAppTree: function(win){
	        ...
	        var appList = dojo.clone(srvApp.appList);
	        var app = nlsApps;//dojo.i18n.getLocalization("scene", "apps");
	        ...
	        var p = new ContentPane({region: "center"});
	        var div = document.createElement("div");
	    ...
	    _handleOpenWith: function(){
	        ...
	        this.scene.launch(this._selectedApp, {file: this.getParent().path+this.name});
	        this._win.close();
	    },
		rename: function(){
			//	summary:
			...
			var evt = dojo.connect(document.body, "onmouseup", this, function(e){
				...
				srvFileSystem.move(this.getParent().path+this.name, value, dojo.hitch(this, function(){
					if(srvConfig.filesystem.hideExt){
		...
		deleteFile: function(e)
		{
			...
			srvFileSystem.remove(this.getParent().path+this.name, dojo.hitch(this, function(){
				var p = this.getParent();
				this.destroy();
				p.layout();
	...
	runtime.addDojoCss("openstar/ui/widget/filearea.css");
