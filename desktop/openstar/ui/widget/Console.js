 define([
  ...
  "../../services/filesystem",
  "../../services/app",
  "dojo/i18n!./nls/console"
],function(dojo,require,_Widget,_Templated,_Contained,utilHtml,srvFilesystem,srvApp,nlsConsole) {

	return dojo.declare( [_Widget,_Templated,_Contained], {
		//	summary:
		...
		aliases: {
			...
			{
				var n = nlsConsole;//dojo.i18n.getLocalization("desktop.widget", "console");
			...
			ps: function(params)
			{
				...
				object = this.app.scene.getInstances();
			...
			kill: function(params)
			{
				...
				if(this.app.scene.kill(params) == 1){ this.write("kill: "+n.procKilled+"\n"); }
			...
			ls: function(params)
			{
				...
				srvFilesystem.listDirectory(params, dojo.hitch(this, function(array){
					var i = 0;
					var out = "";
			...
			mkdir: function(params)
			{
				...
				if(params == ""){
					...
					srvFilesystem.createDirectory(this.path + "/" + params);
				}
				this.detach();
			},
			rm: function(params)
			{
				...
				if(params == ""){
					...
					srvFilesystem.remove(this.path + "/" + params);
				}
				this.detach();
			},
			cat: function(params)
			{
				...
				if(params == ""){
					...
					srvFilesystem.readFileContents(this.path + "/" + params, dojo.hitch(this, function(content){
						this.write(content+"\n");
						this.detach();
					}));
				}
		...
		drawScreen: function(){
			...
			utilHtml.textContent(this.domNode, "");
			dojo.forEach(text.split("\n"), function(val, i){
				var row = document.createElement("div");
				utilHtml.textContent(row, val);
		...
		execApp: function(cmd, params){
			for(var i in srvApp.appList){
				var app = srvApp.appList[i];
				...
				var pid = this.appAttached = this.app.scene.launch(app.sysname, args);
				dojo.connect(this.app.scene.instances[pid], "kill", this, "detach");
				return true;
			}
			return false;
		},
