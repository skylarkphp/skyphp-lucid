	...
	var _SceneBase = Class.declare({
		...
		"-protected-"	:	{
			"-fields-"	:	{
			  ...
			  isLayoutContainer : true,  // for dijit/layout/contentpane
				...
				oldlaunch: function(/*String*/sysname, /*String*/name,/*Object?*/args, /*Function?*/onComplete, /*Function?*/onError){
					//	summary:
					...
					topic.publish("/openstar/desktop2/launchApp", [this,sysname,name]);
					logger.log("launching app "+name);
			        var d = new dojo.Deferred();
			        if(onComplete) d.addCallback(onComplete);
			        if(onError) d.addErrback(onError);

					var segs = sysname.split(":");
					if (segs.length === 3) {
						var promise = runtime.lauchApplication(
							segs[0],
							segs[1],
							segs[2],
							{
		...
		//						icon: app.icon,
							...
							function(instance){
								try {
									instance.init(args||{});
							      	...
							      	topic.publish("/openstar/desktop2/launchAppEnd", [this,sysname,name,false]);
					  				console.error(e);
						        	d.errback(e);
						        	throw e;
				        		  	return;
				        ...
				        var path = "openstar/apps/"+sysname.replace(/[.]/g, "/");
		...
		//						icon: app.icon,
							...
							d.callback(instance);
				   			topic.publish("/openstar/desktop2/launchAppEnd", [this,sysname,name,true]);
						}));
					}
				},
		...
		//modified by LWF 20140718
				resize : function(newSize, currentSize) {
					...
					dojo.forEach(this.getChildren(), function(item){
						item.resize(newSize, currentSize);
					});
				},
